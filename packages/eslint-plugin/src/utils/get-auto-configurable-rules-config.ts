...
function getJsoncRule (rule: string) {
  ...
  const rules = require('./rules').rules as RuleModule[]
  ruleNames ||= new Set(rules.map(r => r.meta.docs.ruleName))

  return ruleNames.has(rule) ? `configurable/${rule}` : null
}

type AutoConfigRules = { [name: string]: Linter.RuleEntry }

/**
 * Get additional jsonc rules config from fileName
 * @param filename
 */
export function getAutoConfig (filename: string): AutoConfigRules {
  const autoConfig: AutoConfigRules = {}
